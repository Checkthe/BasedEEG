...
from utils.evaluation_baselines import evaluate_model
from utils.loading import (loaddata_singal, loaddata_BCI, loaddata_BCICIV2B, loaddata_BCICIV2A)
from config import cfg
from baseline.EEGTransformer import S3T
from baseline.EEGNet import EEGNet
from baseline.ConvNet import DeepConvNet
from baseline.EEG_Inception import EEGInception
...
def train(model,train_loader,epoch):
    ...
    for sn in range(epoch):
        ...
        # print(f"eyes mean: {model.FEB.Frame[0][0].eyes.data.mean().item():.4f}")
        if sn > valid_epoch:
    ...
    # best_model = model_container[max_idx]
    print("=== 本轮评估结果 ===")
...
if __name__ == '__main__':
    ...
    for fold,train_loader in enumerate(train_dataloader):
        ...
        # 创建模型
        model = MSCNN(n_channels=n_channels,n_classes=n_classes)
        model.to(device)
        criterion = nn.CrossEntropyLoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=4e-3, weight_decay=0.05)
        #训练
        epoch = 300
