...
from modeling.CNN import Conv1DClassifier
from modeling.DeepPerceptron import DeepPerceptron
from utils.evaluation_feature import evaluate_model
from utils.loading_feature import load_singal
...
def train(model,train_loader,epoch):
    ...
    for sn in range(epoch):
        ...
        if sn > valid_epoch:
            ...
            del metrics
    # metrics = evaluate_model(model, test_loader=test_loader, detailed_report=True, plot_results=True)
...
if __name__ == '__main__':
    ...
    for fold,train_loader in enumerate(train_dataloader):
        ...
        # 创建模型
        # model = Conv1DClassifier(in_channels=1,num_classes=n_classes)
        model = DeepPerceptron(input_dim=n_points)
        model.to(device)
        ...
        torch.save(model.state_dict(), f'checkpoint/cnn{fold}.pth')
